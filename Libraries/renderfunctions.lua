...
function RenderFunctions:GithubHash(repo, owner)
    local html = httprequest({Url = 'https://github.com/'..(owner or 'Erchobg')..'/'..(repo or 'rendervape')}).Body -- had to use this cause "Arceus X" is absolute bs LMFAO
...
function RenderFunctions:RefreshLocalEnv()
    ...
    for i,v in next, ({'Universal.lua', 'MainScript.lua', 'NewMainScript.lua', 'GuiLibrary.lua'}) do 
        task.spawn(function()
            local contents = game:HttpGet('https://raw.githubusercontent.com/Erchobg/rendervape/main/'..RenderFunctions:GithubHash()..v)
